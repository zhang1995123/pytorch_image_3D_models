...
__all__ = ['Inception3', 'inception_3', 'InceptionOutputs']
...
class Inception3(nn.Module):
    ...
    def _forward(self, x):
        ...
        # N x 32 x 41 x 149 x 149
        x = self.Conv3d_2a_3x3(x)
        # N x 32 x 39 x 147 x 147
        x = self.Conv3d_2b_3x3(x)
        # N x 64 x 39 x 147 x 147
        x = F.max_pool3d(x, kernel_size=3, stride=(1, 2, 2))
        # N x 64 x 37 x 73 x 73
        x = self.Conv3d_3b_1x1(x)
        # N x 80 x 37 x 73 x 73
        x = self.Conv3d_4a_3x3(x)
        # N x 192 x 35 x 71 x 71
        x = F.max_pool3d(x, kernel_size=3, stride=(1, 2, 2), padding=(1, 0, 0))

        # N x 192 x 35 x 35 x 35 下同
        x = self.Mixed_5b(x)
        # N x 256 x 35 x 35 x 35
        x = self.Mixed_5c(x)
        # N x 288 x 35 x 35 x 35
        x = self.Mixed_5d(x)
        # N x 288 x 35 x 35 x 35
        x = self.Mixed_6a(x)
        # N x 768 x 17 x 17 x 17
        x = self.Mixed_6b(x)
        # N x 768 x 17 x 17 x 17
        x = self.Mixed_6c(x)
        # N x 768 x 17 x 17 x 17
        ...
        # N x 768 x 17 x 17 x 17
        x = self.Mixed_7a(x)
        # N x 1280 x 8 x 8 x 8
        x = self.Mixed_7b(x)
        # N x 2048 x 8 x 8 x 8
        x = self.Mixed_7c(x)
        # N x 2048 x 8 x 8 x 8
...
class InceptionC(nn.Module):
    ...
    def __init__(self, in_channels, channels_7x7, conv_block=None):
        ...
        self.branch7x7dbl_4 = conv_block(c7, 192, kernel_size=(1, 1, 7), padding=(0, 0, 3))
...
if __name__=='__main__':
    ...
    input_ = torch.randn(3, 3, 43, 149, 149)
    input_ = input_.cuda()
    (out, aux) = model(input_)
    print(aux.shape)
    print(out.shape)
